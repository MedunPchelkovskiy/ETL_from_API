...
@task(retries=3, retry_delay_seconds=20, task_run_name="get_foreca_data - {place_name}")           #add caching to prevent expensive API calls: cache_key_fn=task_input_hash, cache_expiration=timedelta(hours=1),
...
@task(retries=3, retry_delay_seconds=20, task_run_name="get_accuweather_data - {place_name}")
...
@task(retries=3, retry_delay_seconds=20, task_run_name="get_meteoblue_data - {place_name}")
...
@task(retries=3, retry_delay_seconds=20, task_run_name="get_tomorrow_data - {place_name}")
...
@task(retries=3, retry_delay_seconds=20, task_run_name="get_openweathermap_data - {place_name}")
...
@task(retries=3, retry_delay_seconds=20, task_run_name="get_weatherapi_data - {lat:.2f},{lon:.2f}")
...
@task(retries=3, retry_delay_seconds=20, task_run_name="get_open_meteo_data - {lat:.2f},{lon:.2f}")
