resource "azurerm_container_app_job" "hourly_etl" {
  ...
  secret {
    name                = "client-secret"
    key_vault_secret_id = data.azurerm_key_vault_secret.client_secret.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "db-user"
    key_vault_secret_id = data.azurerm_key_vault_secret.db_user.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }
  ...
  secret {
    name                = "db-conn-raw"
    key_vault_secret_id = data.azurerm_key_vault_secret.db_conn_raw.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "supabase-db-url"
    key_vault_secret_id = data.azurerm_key_vault_secret.supabase_db_url.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "foreca-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.foreca_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "accuweather-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.accuweather_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "meteoblue-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.meteoblue_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "weatherbit-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.weatherbit_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "tommorow-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.tommorow_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "openweathermap-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.openweathermap_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "weatherapi-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.weatherapi_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }

  secret {
    name                = "prefect-api-key"
    key_vault_secret_id = data.azurerm_key_vault_secret.prefect_api_key.id
    identity            = azurerm_user_assigned_identity.etl_jobs.id
  }
